...
class CLEAR1:
    ...
    def wachter_search(self, unit, train_data, training_labels, target_class):
        ...
        loss = np.array([self.model.Pl_loss(CF_space.iloc[i], target_class, self.prototypes) for i in range(len(CF_space))])
    ...
    def test(self):
        ...
        if self.model.predict(self.estimated_b_counterfactual(), self.prototypes) == self.target_class:
            return True
        else:
            return False
...
class CLEAR:
    def __init__(self,model,  num_points_neighbourhood, number_of_counterfactuals,  classification_threshold, regression = None, wachter_search_max = None, learning_rate = None, batch_size = None):
            self.c_t = classification_threshold
            self.num_cf = number_of_counterfactuals
            self.N = num_points_neighbourhood
            self.model = model
            self.wachter_search_max = wachter_search_max
            self.prototypes = model.fit()
            self.lr = learning_rate
            self.batch_size = batch_size
            self.r = regression
    ...
    def synthetic_generator(self, train_data, training_labels):
        ...
        for i in np.unique(training_labels):
            target_indices = np.flatnonzero(training_labels == i)
            target_points = train_data[target_indices]
            mu, sigma = np.mean(target_points, axis = 0), np.cov(target_points.T)
            components.append(rng.multivariate_normal(mu, sigma, number_of_points))
            components_labels.extend([i]*number_of_points)
        ...
        data_new = np.vstack(components)
        data = np.vstack((data_new, train_data))
        labels = np.hstack((np.array(components_labels), training_labels.reshape((training_labels.shape[0],))))
        synthetic_data = np.column_stack((data, labels))
        rng.shuffle(synthetic_data)
        return synthetic_data[:,:synthetic_data.shape[1] - 1], synthetic_data[:,-1]
    ...
    def Balanced_Neighbourhood(self, train_data, training_labels,unit, unit_class, target_class):
        ...
        for i in [unit_class, target_class]:
            ...
            class_distances = []
            for j in class_labels:
                class_distances.append([((abs(unit - synthetic_data[j]))/self.Mean_absolute_deviation(label_data)).sum(), j])
        ...
        neighbourhood_data = np.column_stack((np.array(neighbourhood).reshape(np.array(neighbourhood).shape[0]*np.array(neighbourhood).shape[1], np.array(neighbourhood).shape[2]), np.array(neighbourhood_labels).reshape(self.N, 1)))
        np.random.shuffle(neighbourhood_data)
        neighbourhood_set = neighbourhood_data[:, 0:neighbourhood_data.shape[1] - 1]
        neighbourhood_set_labels = neighbourhood_data[:,-1]
    ...
    def wachter_search(self, unit, train_data, training_labels, target_class):
        ...
        #data = self.normalize(data)
        CF_space = data[np.flatnonzero(labels== target_class)]
        distances = np.array([((abs(unit - CF_space[i]))/self.Mean_absolute_deviation(train_data)).sum() for i in range(len(CF_space))])
        loss = np.array([self.model.Pl_loss(CF_space[i], target_class, self.prototypes) for i in range(len(CF_space))])
    ...
    def generate_counterfactual(self, train_data, training_labels,unit, unit_class, target_class, prototypes = None):
        ...
        FEs = []
        for i in range(len(estimations)):
            fidelity_error = self.fidelity_error(counterfactual_list[i], estimations[i], unit)
            FEs.append(fidelity_error)
            
        Errors = np.array(FEs)
        best_CFEs = Errors.argsort()[:self.num_cf]
        chosen = counterfactual_list[best_CFEs]

        #chosen = pd.DataFrame(best_CFEs, train_data.columns)
        indices = []
        for i in range(chosen.shape[0]):
            if self.check_counterfactual(chosen[i], target_class, prototypes) == True:
                indices.append(i)
        if indices == []:
            print('something went wrong, repeat')
                #print(self.model.proba_predict(chosen.iloc[i], self.prototypes, proto_labels))
